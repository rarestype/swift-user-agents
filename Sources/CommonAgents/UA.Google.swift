...
extension UA {
    @frozen public struct Google {
        public let chrome: ChromeVersion?
        public let type: CrawlerType

        @inlinable public init(chrome: ChromeVersion?, type: CrawlerType) {
            self.chrome = chrome
            self.type = type
        }
    }
        ...
        for component: UA.Component in ua.components {
            ...
            case .product(let product):
                ...
                //  This is a special case for very old Googlebot crawlers.
                    ...
                    case nil = type,
                    let crawler: CrawlerType = .init(
                        prefix: product.name.prefix { $0 != "/" }
                    ) {
                    type = crawler
                }

            case .comment(let comment):
                guard case nil = type else {
                    continue
                }
